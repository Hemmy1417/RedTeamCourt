...
MUTATIONS = [
    ...
    ("an incident closed unresolved still blocks its evidence",
     ...
     "                if prior is None or (\n"),
    ("a filing finalized as rejected still claims its records (boundary)",
     "                        and str(prior.verdict) == \"REJECTED\"):\n",
     "                        and False):\n"),
    ("the registry is global, not per agent (boundary)",
    ...
    ("shared-by-nature categories are registered too",
     ...
     "        if True:\n            self._register_commitment("),
    ("a relabelled controller record is registered only by its label",
     "    return category in REGISTERED_CATEGORIES or origin in UNMINTABLE_ORIGINS\n",
     "    return category in REGISTERED_CATEGORIES\n"),
    ("a relabelled replay is not a reporter record",
    ...
    ("a respondent's unreachable item holds the case",
     ...
     "                and True:\n"),
    ("a reporter-cited respondent record holds the case (boundary)",
     "                and origin[r[\"evidence_id\"]] == ORIGIN_REPORTER:\n",
     "                and submitter[r[\"evidence_id\"]] == ROLE_REPORTER:\n"),
    ("manipulation is the citing party's, not the origin's (boundary)",
     "    return {it[\"evidence_id\"]: ORIGIN_ROLE.get(it[\"origin\"], \"\") for it in ctx[\"items\"]}\n",
     "    return {it[\"evidence_id\"]: it[\"submitter\"] for it in ctx[\"items\"]}\n"),
    ("a manipulation finding accuses ids it never quoted (boundary)",
     "            ids = [q[\"evidence_id\"] for q in f[\"quotes\"]]\n",
     "            ids = f[\"evidence_ids\"]\n"),
    ("the other side's lost item stops an appeal (boundary)",
     "            if rid in stops and state in (ROW_EXAMINED, CHAIN_VERIFIED)\n",
     "            if state in (ROW_EXAMINED, CHAIN_VERIFIED)\n"),
    ("an issuer naming its own origin's party is impersonation (boundary)",
     "    if any(len(name) >= 4 and name in key for name in names.get(origin, [])):\n",
     "    if False:\n"),
    ("a reporter may take a party's name (boundary)",
     "            if _names_overlap(str(reporter.name), other):\n",
     "            if False:\n"),
    ("any chain record verifies a fix (boundary)",
     "    (\"REMEDIATION_VERIFIED\", PRESENT): (\"REMEDIATION_TEST\",),\n",
     ""),
    ("a respondent may request before the deadline (boundary)",
     "            if self._role(incident) != ROLE_REPORTER:\n",
     "            if False:\n"),
    ("a withdrawal request never lapses (boundary)",
     "        if at > _iso_epoch(after) + self._withdrawal_delay(agent, at):\n",
     "        if False:\n"),
    ("a lapsed withdrawal request blocks a new one (boundary)",
     "        if pending != \"\" and at <= _iso_epoch(pending) + delay:\n",
     "        if pending != \"\":\n"),
    ("remediation capacity is shared between parties (boundary)",
