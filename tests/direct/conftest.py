"""Direct Mode harness for contracts/redteam_court.py (see support.py for what
...
of the web, the chain and the model."""
...
from tests.direct.support import CONTRACT, MODULE, NOW, setup_world
...
def _tolerate_windows_unlink():
    ...
    if getattr(original, "_redteam_shim", False):
        return
    ...
    inject_tolerant._redteam_shim = True
...
@pytest.fixture
def genlayer_vm(court):
    import genlayer
    return genlayer.gl.vm


@pytest.fixture
def world_ids(court, direct_vm):
    """The policy, the tool, the agent and the reporters registered, the agent
    wallet confirmed, the bond and the bounty pool funded."""
    return setup_world(court, direct_vm)
